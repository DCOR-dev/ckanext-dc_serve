...
def generate_condensed_resource_job(resource, override=False):
    ...
    with CKANResourceFileLock(resource_id=resource["id"],
        ...
        # we can just ignore this resource. The reason why I implemented
        # this is because I wanted to add an automated background job
        # for generating missing condensed files, but then several processes
        # would end up condensing the same resource.
        if fl.is_locked:
